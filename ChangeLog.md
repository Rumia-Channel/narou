...
----------
#### 修正
- Linux で大量の並列変換を行うと ThreadPool の Ruby スレッドスタックが膨れ上がり、大量のメモリ・CPU を消費していた問題を修正。デフォルトの Linux 同時変換数を最大8に制限し、Web UI 実行時や総メモリ8GB以下/厳しい cgroup 制限下では自動的に 1～2 スレッドまで絞り込みます。環境変数 `NAROU_THREAD_POOL_SIZE` で明示設定も可能で、処理完了後はスレッドを確実に破棄してメモリを即座に解放します。
- 変換キャッシュ (section_convert_cache) をメモリ常駐させず、各セクションの内容をディスクに退避する方式へ変更。大量の小説を連続変換しても Ruby プロセスが数 GB 規模まで肥大化しなくなり、従来のキャッシュファイルも変換処理中に自動移行されます。
- Web UI 経由の変換 API が concurrency モード有効時にアプリサーバースレッドをブロックしてしまい、Web UI ごとフリーズしていた問題を修正。常にバックグラウンドワーカーにジョブを積むように変更し、リクエスト応答と Push 通知を即時返せるようになりました。
