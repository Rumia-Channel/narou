...
var Narou = (function () {
  ...
  $.extend(Notification.prototype, {
    ...
    create_ws_uri: function() {
      // HTTPS→wss, HTTP→ws を自動選択し、
      // ポートは location.host（デフォルト 443 は省略済み）を使う
...
<label><input type="checkbox" id="update-gl-and-update-modified" <% if (updateModified) { %>checked<% } %>> 確認後にmodifiedタグが付いた小説をUpdate</label>
  ...
  $.extend(Console.prototype, {
    ...
    init_scrollbar: function () {
      if (this.console.length === 0) return;
      this.ps = new PerfectScrollbar(this.console[0], {
        wheelSpeed: 1,  // v1.xでは wheelspeed -> wheelSpeed。値も調整(0.5-2程度)
        wheelPropagation: false, // suppressScrollX相当? いや suppressScrollXはある
    ...
    initializeProgressBar: function (percent, topic) {
      var key = topic || "default";
      if (this.progressBars && this.progressBars[key]) {
        this.removeProgressBar(key);
      }
      this.puts(this.createProgressHtml(percent));
      var bar = this.console.find(".progress-bar").last();
      if (!this.progressBars) this.progressBars = {};
      this.progressBars[key] = bar;
    },

    setProgressValue: function (percent, topic) {
      var key = topic || "default";
      if (!this.progressBars || !this.progressBars[key])
        this.initializeProgressBar(percent, topic);
      this.progressBars[key].width(percent + "%");
    },

    removeProgressBar: function (topic) {
      var key = topic || "default";
      if (!this.progressBars || !this.progressBars[key]) return;
      this.progressBars[key].parent().parent().remove();
    ...
    init_events_progressbar: function () {
      ...
      this.notification.on("progressbar.init", function (json) {
        ...
        self.initializeProgressBar(0, json.topic);
      });
      this.notification.on("progressbar.step", function (json) {
        if (json.target_console !== self.options.stream_id) return;
        self.setProgressValue(json.percent, json.topic);
      });
      this.notification.on("progressbar.clear", function (json) {
        if (json.target_console !== self.options.stream_id) return;
        self.removeProgressBar(json.topic);
      });
    },
