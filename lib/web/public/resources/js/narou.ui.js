...
$(function() {
  ...
  // 端末ごとの表示項目初期設定
  var is_small_display = $(window).width() < 768;
  var cell_visible = (touchable_device && is_small_display ?
      // スマフォ系端末
      {
  ...
  function initializeDataTable(serverSortState) {
    ...
    stateLoadCallback: function(settings, callback) {
      try {
        ...
        // タッチデバイスでは ColReorder が無効なので、保存された ColReorder 状態を削除
        // これにより、PCで変更されたカラム順序がスマホ/タブレットに影響しない
        if (touchable_device && state.ColReorder) {
          console.log("[DEBUG] Removing ColReorder state for touch device");
    ...
    initComplete: function(_settings, _json) {
      ...
      // タッチデバイスでは ColReorder 状態を保存しない
      // これにより、スマホ/タブレットでの操作が PC のカラム順序に影響しない
      if (touchable_device) {
        table.on('stateSaveParams.dt', function(e, settings, data) {
          if (data.ColReorder) {
            console.log("[DEBUG] Preventing ColReorder state save on touch device");
  ...
  */
  ...
  $('#action-view-setting').on(click_event_name, function(e) {
    ...
    $collection.addClass('dropdown-menu dt-colvis-menu');

    // インラインスタイルの背景色を明示的に削除
    ...
    // ボタン内のアイテムにBootstrapのスタイルを適用
    $collection.find('button').each(function() {
      var $btn = $(this);
      // インラインスタイルの背景色を削除
      ...
      // DataTablesのボタンクラスを保持しつつ、Bootstrap互換のスタイルを追加
      if (!$btn.hasClass('dt-button-background')) {
        $btn.css({
          'display': 'block',
          'width': '100%',
          'text-align': 'left',
