...
$(function() {
  ...
  function initializeComponents() {
    ...
    // Shiftクリックによる意図しないテキスト選択を防ぐ（必要な機能は除外）
    ...
    document.addEventListener('selectstart', function(e) {
      if (e.shiftKey) {
        ...
        // その他すべての要素で選択を防ぐ
        console.log('Preventing text selection on shift+click for:', target);
        e.preventDefault();
        e.stopPropagation();
    ...
    // マウスダウン時のShiftクリックでテキスト選択を防ぐ
    document.addEventListener('mousedown', function(e) {
      if (e.shiftKey) {
        // タグ要素のクリックかチェック
        var tagElement = $(e.target).closest('.tag[data-tag]:not(.tag-reset)')[0];
        if (tagElement) {
          return; // タグクリックでは通常処理を継続（Shift機能を維持）
        }
